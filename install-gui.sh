...
if [ "$(id -u)" = 0 ]; then
  echo "The Zevs Blockchain GUI can not be installed or run by the root user."
	exit 1
fi

# Allows overriding the branch or commit to build in Zevs-blockchain-gui
SUBMODULE_BRANCH=$1
...
echo ""
echo "Zevs blockchain install-gui.sh completed."
echo ""
echo "Type 'cd Zevs-blockchain-gui' and then 'npm run electron &' to start the GUI."
